...
<main class="login-pattern flex min-h-screen items-center justify-center overflow-hidden px-4 py-8 sm:px-8 lg:px-12">
    <div class="grid w-full max-w-6xl overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xl shadow-slate-200/70 lg:grid-cols-12">
        <section class="relative flex min-h-[460px] flex-col justify-center overflow-hidden bg-gradient-to-br from-[#1e3a8a] via-[#1e40af] to-[#172554] px-8 py-10 text-white sm:px-11 lg:col-span-5">
            <img src="{{ asset('images/login-campus.png') }}" alt="Gedung sekolah SMA Mardisiswa" class="absolute inset-0 h-full w-full object-cover opacity-100 mix-blend-overlay">
            <div class="absolute inset-0 bg-gradient-to-t from-[#172554]/95 via-[#1e3a8a]/75 to-[#1e40af]/55 opacity-100"></div>
        ...
        <section class="flex flex-col justify-center px-7 py-10 sm:px-12 lg:col-span-7 lg:py-16">
            <div class="mb-8 flex items-start justify-between gap-4"><div><h2 class="text-2xl font-extrabold tracking-tight text-slate-900 sm:text-3xl">Masuk ke Akun Anda</h2><p class="mt-2 text-xs leading-5 text-slate-500 sm:text-sm">Silakan masukkan kredensial resmi sekolah untuk melanjutkan.</p></div></div>
            ...
            <form method="POST" action="{{ route('login.store') }}" class="space-y-5">@csrf
                <div><label for="identity" class="mb-2 block text-xs font-bold text-slate-700">NIP</label><div class="relative"><span class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3.5 text-slate-400">👤</span><input id="identity" name="identity" value="{{ old('identity') }}" required autofocus autocomplete="username" placeholder="Masukkan NIP" class="w-full rounded-lg border border-slate-300 bg-slate-50 py-3 pl-10 pr-4 text-sm font-medium text-slate-800 outline-none transition focus:border-blue-600 focus:bg-white focus:ring-2 focus:ring-blue-100"></div></div>
                <div><div class="mb-2 flex items-center justify-between"><label for="password" class="text-xs font-bold text-slate-700">Kata Sandi</label><a href="#" class="text-xs font-semibold text-blue-700 hover:underline">Lupa kata sandi?</a></div><div class="relative"><span class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3.5 text-slate-400">🗝️</span><input id="password" name="password" type="password" required autocomplete="current-password" placeholder="Masukkan kata sandi" class="w-full rounded-lg border border-slate-300 bg-slate-50 py-3 pl-10 pr-12 text-sm font-medium text-slate-800 outline-none transition focus:border-blue-600 focus:bg-white focus:ring-2 focus:ring-blue-100"><button type="button" data-password-toggle class="absolute inset-y-0 right-0 px-4 text-slate-400" aria-label="Tampilkan kata sandi">◉</button></div></div>
                ...
                <button type="submit" class="flex w-full items-center justify-center gap-2 rounded-xl bg-[#1e3a8a] px-4 py-3.5 text-sm font-bold text-white shadow-lg shadow-blue-900/25 transition hover:-translate-y-0.5 hover:bg-[#1e40af]">Masuk</button>
            </form>
        </section>
    </div>
</main>
